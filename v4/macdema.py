...
4. Learn to hedge
5. Basket closure strategy (based on current number of trades? - 100 per lot)
...
for line in range(0, len(to_trade_final)):
    if to_trade_final['Trend'].loc[line] == 'sell':
        sl_raw = to_trade_final['Current Price'].loc[line] + 3*to_trade_final['atr'].loc[line]
        sl.append(sl_raw)
        tp_raw = to_trade_final['Current Price'].loc[line] - 5*to_trade_final['atr'].loc[line]
        tp.append(tp_raw)
