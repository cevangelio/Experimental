...
v3 - follow trend but price is already beyond previous low/high
...
def trade_odin(port, vol):
    ...
    if con == True:
        for currency in to_trade_final_raw['Currency']:
            ...
            coms = 'odin_v3_1135_'+port_dict[port]
...
if datetime.now().hour >= 19:
    ...
    if morning < 0:
        telegram_bot_sendtext('Odin defeated in the morning. ')
        trade_odin(port = 1135, vol = round(fxcm_demo, 2))
        ...
        # trade_odin(port = 1122, vol = ftmo_vol, rev='FT')
        trade_odin(port = 1135, vol = fxcm_demo)
