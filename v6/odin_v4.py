...
v4 - meah reverse of start ft but previous low/high
...
MT = Pytrader_API()
ports = [1122, 1125, 1127, 1129, 1131, 1133,1137]
port_dict = {1122:'FTMO_Live', 1125:'FXCM_Demo', 1127:'GP_Live', 1129:'GP_Demo', 1131:'FTMO_Demo', 1133:'FTMO_DEMO_2',1137:'FXCM_DEMO_V4'}
...
con = MT.Connect(server='127.0.0.1', port=1137, instrument_lookup=symbols)
...
def trade_odin(port, vol):
    ...
    if con == True:
        for currency in to_trade_final_raw['Currency']:
            ...
            coms = 'odin_v4_1137_'+port_dict[port]
...
#     for currency in to_trade_final_raw['Currency']:
#         dirxn = to_trade_final_raw['Action'][to_trade_final_raw['Currency'] == currency].values[0]
#         coms = 'ODN_v2_MR_'+port_dict[1133]
#         order = MT.Open_order(instrument=currency, ordertype=dirxn, volume=fxcm_demo, openprice = 0.0, slippage = 10, magicnumber=1133, stoploss=0, takeprofit=0, comment =coms)
#         time.sleep(1)
#         print(currency, order)
#         if order == -1:
#             telegram_bot_sendtext('ODIN - ERROR opening order for '+ currency + '-'+ dirxn.upper())
# else:
#     telegram_bot_sendtext('Odin failed for '+port_dict[1133] + '.')
...
if datetime.now().hour >= 19:
    ...
    if morning < 0:
        ...
        trade_odin(port = 1137, vol = fxcm_demo)
